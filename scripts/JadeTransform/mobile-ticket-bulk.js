...
module.exports = {
    mobileTicketBulkInstall : function(config){
        var deferred = Q.defer();
        var edx_gateway_ip = process.env.EDX_GATEWAY_IP;
        if (edx_gateway_ip === undefined) {
            edx_gateway_ip = config.Companies_products_bulkticket_settings_dataEndPoints_EDXTicketService_EndPointAddress;
        }
        ...
        prompt.get(promptEDXGatewayIp, function (err, result) {
            ...
            configMongo.Companies_products_bulkticket_settings_dataEndPoints_EDXTicketService_EndPointAddress = result.edx_gateway_ip;

            var edx_gateway_port = process.env.EDX_GATEWAY_PORT;
            if (edx_gateway_port === undefined) {
                edx_gateway_port = config.Companies_products_bulkticket_settings_dataEndPoints_EDXTicketService_Port;
            }
            ...
            prompt.get(promptEDXGatewayPort, function (err, result) {
                ...
                configMongo.Companies_products_bulkticket_settings_dataEndPoints_EDXTicketService_Port = result.edx_gateway_port;
