...
describe('Test Prompts', function(){
    ...
    describe('Non product info test for new customer', function(){
        ...
        describe('Company status', function(){
            ...
            it('should match only y[es] or n[o] ', function(){
                expect(transformProperties.companyObj.status).to.match(/^(y[es]*|n[o]*)$/);
            });
        });
        ...
        describe('Company email', function(){
            ...
            it('should be a valid email address ', function(){
                expect(transformProperties.companyObj.settings_communications_email_from_value).to.match(/^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/);
            });
        });
        ...
        describe('Company cloud storage', function(){
            ...
            it('should match only y[es] or n[o] ', function(){
                expect(transformProperties.companyObj.settings_cloudStore).to.match(/^(y[es]*|n[o]*)$/);
            });
        });
